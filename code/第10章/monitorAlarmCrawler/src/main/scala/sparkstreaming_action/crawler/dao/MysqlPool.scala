package sparkstreaming_action.crawler.dao

import java.sql.Connection
import com.mchange.v2.c3p0.ComboPooledDataSource
import sparkstreaming_action.crawler.util.Conf
...
class MysqlPool extends Serializable {
  ...
  try {
    cpds.setJdbcUrl(conf.get("url").getOrElse("jdbc:mysql://localhost:3306/monitor_alarm?useUnicode=true&amp;characterEncoding=UTF-8"));
    cpds.setDriverClass("com.mysql.jdbc.Driver");
    cpds.setUser(conf.get("username").getOrElse("root"));
    cpds.setPassword(conf.get("password").getOrElse("root"))
